...
export default function Comment({ dish }: any) {
  ...
  const { user, tenantId, tenantSlug } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      setIsloading(true);
      const commentClass = new CommentClass(tenantId);
      const result: any = await commentClass.getByDishId(dish.id);
      setComments(result);
      setIsloading(false);

    })();
  }, [dish.id, updated, tenantId]);

  const handleReadMore = async (event: any) => {
    event.preventDefault();
    setStart(start + limit);
  }


  const createNotification = async (commentData: any, action = "create") => {
  ...
  const handleChangeComment = async (e: any, type: string, value: any) => {
    try {
      ...
      if (value) {
        ...
        if (currentComment) {
          ...
          const result: any = await commentClass.insert({
            ...inputData,
            createdBy: user.id,
            dishId: dish.id,
            status: "publish"
  ...
  const handleUploadFile = async (file: any, path: string) => {
    try {
      path = `/${tenantSlug}/comments/${path}`;
  ...
  const handleCommentAction = (action: string, commentData = false) => {
    if (action === "edit") {
      handleEditComment();
  ...
  return (
    <div>
      <CommentInput comment={editComment} type="Comment" handleSubmit={handleChangeComment} count={comments.length} />
      <List
        // ref={commentRef}
        sx={{
          width: '100% !important',
      ...
      >
        {comments?.length ?
          comments.slice(0, start + limit).map((menuComment, key) => {
                    ...
                    primary={
                      <>
                        <Typography component='span' sx={{ fontWeight: "bold", width: "100%" }}>
                          {menuComment.userData?.role ? "Response by owner" : commentUser.name || "No Name"}
            ...
            >
              <Typography component='p' fontWeight='700' >No comments yet</Typography>
...
const CommentContent = ({ menuComment }: any) => {
    ...
    <>
      {
        type === "image" ? (
          <span>
            <Image
              src={url || "/x612.jpg"}
              // placeholder={"/menu-placeholder.jpeg"}
              alt={"Comment Image"}
              width={200}
              height={150}
              loading="lazy"
