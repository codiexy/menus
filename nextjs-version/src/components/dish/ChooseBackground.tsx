...
export default function ChooseBackground({ imageUrl }: any) {

  const imageFile = imageUrl;

  const [image, setImage] = useState("");

  const removeBackgroundWithWhite = async (imageUrl: string) => {
    const options = {
      method: 'GET',
  ...
  const removeBackgroundWithTransparent = async (imageUrl: any) => {
    const options = {
      method: 'GET',
      url: 'https://beta-sdk.photoroom.com/v1/render',
  ...
  const backGroundChangeImage = async (imageUrl: any) => {
    const options = {
      method: 'GET',
