...
const DefaultSuggestions = ({ dishesData }: any) => {
  ...
  return (
    <Grid sx={{ ml: 0 }}>
      ...
      {dishesData.length ? (
        <>
          {dishesData.map((item: any, index: any) => (
            <Grid item xs={12} sm={12} key={index}>
              <List sx={{ py: 1 }}>
                <ListItem sx={{ py: 0 }} disablePadding>
                  <Link href={`/${tenantSlug}/${item.id}`} style={{ textDecoration: "none", cursor: "pointer", color: "#4458BE" }}>
