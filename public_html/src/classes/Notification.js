...
export default class Notification extends Database {
    ...
    getAll = async (options = {}) => {
        ...
        if (pagination === true) {
            ...
            if (result.status) {
                const data = result.data.map(async (like) => await this._formatData(like))
    ...
    _getReplyData = async (data) => {
        switch (data.action) {
            case "create":
                data.icon = <ReplyIcon />;
                data.title = "New Reply";
                data.description = (
                    <>New comment's reply in {data.parentName} dish menu.</>
                )
                break;
            case "update":
                data.icon = <EditIcon />;
                ...
                data.description = (
                    <>Update comment's reply in {data.parentName} dish menu.</>
                )
                break;
            case "delete":
                data.icon = <DeleteForeverIcon />;
                ...
                data.description = (
                    <>Comment's reply deleted in {data.parentName} dish menu.</>
                )
                break;
            default:
                break;
