...
export default function BasicMenu({ id, tableCollection = "FactoryDogs", setIsLoader }) {

  const handleChange = async (event) => {
    const input = event.target.files[0];
    if (!input) toast(`File not selected!`, { type: "warning" });
    setIsLoader(true);
    const docRef = doc(db, tableCollection, id);
    ...
    <div>
      <Tooltip title={`Update photo`}>
        <IconButton
          ...
          color="secondary" aria-label="upload picture" component="label"
        >
